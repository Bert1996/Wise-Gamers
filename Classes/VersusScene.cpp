...
bool VersusScene::onTouchBegan(cocos2d::Touch* touch, cocos2d::Event* event) {
...
void VersusScene::onTouchEnded(cocos2d::Touch* touch, cocos2d::Event* event) {
...
void VersusScene::player1PlayerSpin()
{
	float s = sin(0.1);
	float c = cos(0.1);
...
void VersusScene::player1MovePlayer()
{
	auto size = Director::getInstance()->getVisibleSize();
...
void VersusScene::player1ScrollingBackground()
{
	auto size = Director::getInstance()->getVisibleSize();
...
void VersusScene::player1SetRobotRotation()
{
	Vec2 cog = _player1closeCog;
	Vec2 play = player1->getPosition();
...
void VersusScene::player2CheckForClosest()
{
	...
	Vec2 play1 = player2->getPosition();
...
void VersusScene::player2WallCollide()
{
	auto size = Director::getInstance()->getVisibleSize(); //27 ! 72
	float leftWall = ((size.width / 100) * 53.45);
...
void VersusScene::player2PlayerSpin()
{
	float s = sin(0.1);
	float c = cos(0.1);

	Vec2 point = _player2closeCog;
	Vec2 Player = player2->getPosition();
	Vec2 playerpos = Player;

	playerpos -= point;

	if (!_player2clockwise)
	{
		player2->setFlipY(true);
		player2flipped = true;
		//AntiClockwise Spin
		float x = playerpos.x * c - playerpos.y * s;
		float y = playerpos.x * s + playerpos.y * c;
...
void VersusScene::player2MovePlayer()
{
	auto size = Director::getInstance()->getVisibleSize();
	float moveY = ((size.height / 100) * 19);
	...
	//Player Movement
	//x
	float rot = cos(CC_DEGREES_TO_RADIANS(player2->getRotation() - 90));
	...
	if (player2flipped)
		player2->setPosition(player2->getPosition().x + -(rot * 4), player2->getPosition().y);

	if (!player2flipped)
		player2->setPosition(player2->getPosition().x + (rot * 4), player2->getPosition().y);

	//y
	if (player2->getPosition().y != moveY && player2->getPosition().y > moveY) //19% of size.y
	{
		player2->setPosition(player2->getPosition().x, player2->getPosition().y - 2.0f);
	}
	if (player2->getPosition().y != moveY && player2->getPosition().y < moveY)
	{
		player2->setPosition(player2->getPosition().x, player2->getPosition().y + 2.0f);
	}
...
void VersusScene::player2ScrollingBackground()
{
	auto size = Director::getInstance()->getVisibleSize();
...
void VersusScene::player2SetRobotRotation()
{
	Vec2 cog = _player2closeCog;
	Vec2 play = player2->getPosition();
